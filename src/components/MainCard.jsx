...
import { faCircleInfo, faGreaterThan, faLessThan, faSpinner } from '@fortawesome/free-solid-svg-icons';
  ...
  return (
    <div className="flex justify-center items-center space-x-4 my-4">
      {[1, 2, 3, 4].map((step, idx) => (
        <button
          key={idx}
          className={`rounded-full w-10 h-10 ${currentStep === step ? 'bg-teal-600' : 'bg-teal-500'} text-white flex items-center justify-center text-xl font-bold hover:bg-teal-600 transition-all duration-300 border-solid border-2 border-black`}
...
const MainCard = () => {
  ...
  const handleNext = async () => {
    if (currentCard === 1) {
      ...
      try {
        const response = await fetch('/api/output', { // Backend endpoint
          method: 'POST',
          headers: {
  ...
  return (
    <div className="main-card flex flex-col items-center mx-4 max-w-screen-lg">
      <StepIndicator currentStep={currentCard} setCurrentStep={setCurrentCard} /> {/* Include Step Indicator at the top */}
      ...
      <div className="flex flex-col lg:flex-row items-center justify-between w-full">
        ...
        <button
          ...
          onClick={handlePrev}
        >
          <span className="mr-2"><FontAwesomeIcon icon={faLessThan} /></span>
        </button>
            ...
            <div className="bg-white bg-opacity-35 backdrop-blur-md rounded-[50px] shadow-xl p-8 max-w-3xl  w-full flex flex-col lg:flex-row items-center justify-between">
              <div className="flex flex-col justify-center space-y-4">
                ...
                <div className="flex items-center space-x-2 pt-8">
                  <select className="bg-gray-200 p-2 rounded-md" value={aiModel} onChange={(e) => setAiModel(e.target.value)}>
              ...
              <div className="flex flex-col space-y-6 pb-4 px-5">
                {['Video Script', 'Course Content', 'Project'].map((item, idx) => (
                  <button
                    key={idx}
                    className={`rounded-lg text-white text-2xl font-semibold p-4 hover:bg-teal-600 transition ${contentType === item ? 'bg-teal-600' : 'bg-teal-500'}`}
                    onClick={() => {
                      setContentType(item);
                      setCurrentCard(2);
                    }}
          ...
          {currentCard === 2 && (
            <div className="bg-white bg-opacity-35 backdrop-blur-md rounded-[50px] shadow-xl p-8 max-w-3xl w-full flex flex-col lg:flex-row items-center justify-between h-[300px]">
              ...
              <div className='flex flex-col space-y-6 pb-4 px-5'>
                ...
                />
                <input type="file" accept=".csv" onChange={handleFileChange} className="mb-4 flex flex-col" />
              </div>
            </div>
          )}
        ...
        <button
          ...
          onClick={handleNext}
        >
          <span className="mr-2"><FontAwesomeIcon icon={faGreaterThan} /></span>
        </button>
      </div>
    </div>
  );
